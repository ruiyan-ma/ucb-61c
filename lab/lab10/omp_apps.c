...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
    ...
    #pragma omp parallel
    {
        ...
        for (int i = tid; i < ARRAY_SIZE; i += total)
            z[i] = x[i] + y[i];
    }
}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
    ...
    #pragma omp parallel
    {
        int total = omp_get_num_threads();
        int tid = omp_get_thread_num();
        int step = ARRAY_SIZE / total + 1;
        for (int i = tid * step; i < (tid + 1) * step && i < ARRAY_SIZE; i++)
            z[i] = x[i] + y[i];
    }
}
// END PART 1 EX 2
...
double dotp_manual_optimized(double* x, double* y, int arr_size) {
    ...
    #pragma omp parallel
    {
        int total = omp_get_num_threads();
        int tid = omp_get_thread_num();
        int step = arr_size / total + 1;
        double local_sum = 0.0;

        for (int i = tid * step; i < (tid + 1) * step && i < arr_size; i++)
            local_sum += x[i] * y[i];
