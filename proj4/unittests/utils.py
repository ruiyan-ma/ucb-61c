...
def rand_dp_nc_matrix(rows, cols, seed=0):
    np_mat = dp.zeros((rows, cols))
    nc_mat = nc.Matrix(rows, cols, rand=True, seed=seed)
    # set np_mat the same as nc_mat
    for i in range(rows):
        for j in range(cols):
            np_mat[i][j] = nc_mat[i][j]
...
def compute(dp_mat_lst: List[Union[dp.matrix, int]],
    ...
    if op == "neg" or op == "abs":
        ...
        for mat in dp_mat_lst[1:]:
            if op == "mul":
                dp_result = np.matmul(dp_result, mat)
            elif op == "pow":
                dp_result = np.linalg.matrix_power(dp_result, mat)
            else:
